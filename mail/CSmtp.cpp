	...
	try
	{
		...
		if(authenticate && IsKeywordSupported(RecvBuf, "AUTH") == true)
		{
			...
			else if(IsKeywordSupported(RecvBuf, "PLAIN") == true)
			{
				...
				{
					if (SendBuf[i] == 94) SendBuf[i] = 0;
				}
				std::string encoded_login = base64_encode(reinterpret_cast<const unsigned char*>(SendBuf), length);
			...
			else if(IsKeywordSupported(RecvBuf, "CRAM-MD5") == true)
			{
				...
				const char *ustrChallenge = decoded_challenge.c_str();
				const char *ustrPassword = m_sPassword.c_str();
				// if ustrPassword is longer than 64 bytes reset it to ustrPassword=MD5(ustrPassword)
				int passwordLength=m_sPassword.size();
				if(passwordLength > 64){
					MD5 md5password;
					md5password.update(decoded_challenge.c_str(), passwordLength);
					md5password.finalize();
					ustrPassword = md5password.hexdigest().c_str();
					passwordLength = 16;
				}
				...
				md5pass1.finalize();
				const char *ustrResult = md5pass1.hexdigest().c_str();
				...
				md5pass2.finalize();
				decoded_challenge = md5pass2.hexdigest().c_str();
			...
			else if(IsKeywordSupported(RecvBuf, "DIGEST-MD5") == true)
			{
				...
				const char *ustrRealm = realm.c_str();
				const char *ustrUsername = m_sLogin.c_str();
				const char *ustrPassword = m_sPassword.c_str();
				const char *ustrNonce = nonce.c_str();
				const char *ustrCNonce = cnonce;
				const char *ustrUri = uri.c_str();
				const char *ustrNc = nc;
				const char *ustrQop = qop.c_str();
				...
				md5a1a.finalize();
				const char *ua1 = md5a1a.hexdigest().c_str();
				...
				md5a1b.finalize();
				const char *a1 = md5a1b.hexdigest().c_str();
				...
				md5a2.finalize();
				const char *a2 = md5a2.hexdigest().c_str();
				...
				md5.update(a2, 32);
				md5.finalize();
				decoded_challenge = md5.hexdigest().c_str();
				...
				strcat(SendBuf, RecvBuf);
				encoded_challenge = base64_encode(reinterpret_cast<const unsigned char*>(SendBuf), strlen(SendBuf));
				snprintf(SendBuf, BUFFER_SIZE, "%s\r\n", encoded_challenge.c_str());
				pEntry = FindCommandEntry(command_DIGESTMD5);
				SendData(pEntry);
				ReceiveResponse(pEntry);
